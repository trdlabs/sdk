 ...
 */
export const EVENT_DRIVEN_MIN_CONTRACT_VERSION = '017.5' as const;
...
interface OpenOrderViewBase {
  ...
  /**
   * ПЕРВИЧНАЯ величина заявки (ADR-0013, `017.5`): запрошенный нотионал в USD — то, что автор
   * попросил, в той единице, в которой попросил (см. `ActorPlaceCommand.qtyUsd`).
   */
  readonly qtyUsd: number;
  /**
   * Исполненная часть В ТОЙ ЖЕ единице — ФАКТ: сумма `qty × price` по филлам этой заявки.
   *
   * Остаток заявки — `qtyUsd − filledQtyUsd`, обе части одной единицы, и никакой цены для их
   * сравнения не требуется. Предикат частичного исполнения — `0 < filledQtyUsd < qtyUsd`.
   */
  readonly filledQtyUsd: number;
  /**
   * ОЦЕНКА размера в базовой валюте — сколько это примерно в единицах инструмента.
   *
   * НЕОБЯЗАТЕЛЬНА намеренно: у хоста может не быть цены пересчёта (заявка подана до первой
   * доставленной цены), и честный ответ тогда «не знаю», а не число. Обязательством не является —
   * исполнение придёт по другой цене, и `filledQty` с этой оценкой не совпадёт.
   *
   * Здесь стояло поле `qty`, объявленное «размером, ПРИНЯТЫМ риском». Утверждение оказалось
   * невыполнимым: у actor-пути риск-контура нет вовсе, то есть числа, принятого риском, не
   * существует, а хост, у которого нет цены, подставлял ноль — и по прежней формуле остатка
   * (`qty − filledQty`) это читалось как «исполнена целиком», то есть автор видел заявку, которой
   * уже нет.
   */
  readonly estimatedQty?: number;
  /**
   * Фактически исполненный размер в базовой валюте. Присутствует ⟺ по заявке были исполнения.
   *
   * ФАКТ, в отличие от `estimatedQty` выше: он сложен из реальных филлов и ни на какую цену не
   * опирается.
   */
 ...
 */
export interface ActorMarketSourceDescriptor {
  readonly subscriptionId: SubscriptionId;
...
/**
 * ХОСТОВЫЙ источник — события, порождённые не лентой, а самим хостом (ADR-0012, `017.5`).
 *
 * ЗАЧЕМ ОН СУЩЕСТВУЕТ. `ActorEnvelope` требует `subscriptionId` у КАЖДОГО события, а половина
 * каталога §3.5 рождается внутри хоста: `fill`, четыре ордерных события, `timer.fired`. Подписки,
 * породившей их, нет и быть не может — у филла нет требования манифеста.
 *
 * До `017.5` взять значение было неоткуда, и хост-реализация (083 S3) подставляла собственную
 * строку. Это работает ровно до второго хоста: автор, проверяющий `subscriptionId` по списку
 * источников — а контракт разрешает ему именно это, — увидел бы неизвестный идентификатор и по
 * своей же логике обязан был бы счесть событие подложным. Второй хост выбрал бы другую строку, и
 * один и тот же код стратегии повёл бы себя на бэктесте и в проде по-разному, молча.
 *
 * `requirementId` здесь НЕТ, и это не пропуск: требования манифеста у хостового источника не
 * существует. Именно поэтому дескриптор разложен на два варианта, а не получил опциональное поле —
 * опциональность позволила бы рыночному источнику приехать без требования и не быть замеченным.
 */
export interface ActorHostSourceDescriptor {
  readonly subscriptionId: SubscriptionId;
  readonly kind: 'host';
}

/**
 * Один разрешённый источник событий актора — элемент `ActorInit.subscriptions`.
 *
 * СПИСОК ЗАКРЫТ (ADR-0012). Правило автора ОДНО и без исключений: `ActorEnvelope.subscriptionId`
 * всегда присутствует в этом списке. Альтернатива — зарезервированное значение с оговоркой
 * «сравнивай со списком, КРОМЕ вот этой строки» — держалась бы документацией, а не типом, и первый
 * же автор, забывший про исключение, отверг бы собственный филл.
 */
export type ActorSubscriptionDescriptor = ActorMarketSourceDescriptor | ActorHostSourceDescriptor;

/** Хостовый ли это источник. Сужение по дискриминанту — читателю не надо помнить состав union'а. */
export function isHostSourceDescriptor(
  descriptor: ActorSubscriptionDescriptor,
): descriptor is ActorHostSourceDescriptor {
  return descriptor.kind === 'host';
}

/**
 * КАНОНИЧЕСКИЙ идентификатор хостового источника. Выбирает его КОНТРАКТ, а не хост.
 *
 * Зачем константа, когда тип уже разрешает любую строку. Идентификатор наблюдаем автором: он
 * приезжает в каждом конверте хостового события, автор кладёт его в свой FSM и по нему ветвится.
 * Если значение выбирает хост, то бэктестер, платформа и живая торговля выберут разные — и один и
 * тот же код стратегии поведёт себя в трёх средах по-разному, причём молча: ни одна проверка формы
 * этого не заметит, потому что форма у всех трёх одинаковая.
 *
 * Ровно это и случилось в 083 S3: бэктестер придумал себе `'sub-internal'`, потому что взять
 * значение было неоткуда. Тип, разрешающий всё, — не контракт, а приглашение к расхождению.
 *
 * Значение стабильно и частью версии контракта не является: его смена — ломающая правка того же
 * класса, что переименование поля.
 */
export const HOST_SUBSCRIPTION_ID: SubscriptionId = 'host';

/**
 * Готовый дескриптор хостового источника — тот, что хост обязан положить в
 * `ActorInit.subscriptions`.
 *
 * Заморожен и единственен: два экземпляра с одинаковым содержимым — это два места, где значение
 * можно разойтись, а сравнение по ссылке перестаёт работать там, где на него понадеялись.
 */
export const HOST_SOURCE_DESCRIPTOR: ActorHostSourceDescriptor = Object.freeze({
  subscriptionId: HOST_SUBSCRIPTION_ID,
  kind: 'host',
});

/**
 * Хостовый ли это ИДЕНТИФИКАТОР — для мест, где дескриптора под рукой нет, а есть только конверт.
 *
 * Отдельная функция, а не сравнение со строкой у вызывающего: сравнение расползается по коду, и
 * когда значение однажды сменится, часть сравнений останется прежней.
 */
export function isHostSubscriptionId(subscriptionId: SubscriptionId): boolean {
  return subscriptionId === HOST_SUBSCRIPTION_ID;
}
 ...
 * батч.
 *
 * **Точность гарантии (уточнение после релиза 0.14.0): «ни тип не ловит» было пессимистичнее
 * факта.** СВЕЖИЙ объектный литерал, переданный НАПРЯМУЮ в `defineActor(...)`, ловится
 * excess-property-check компилятора — `defineActor({ onTimer: () => [] })` (опечатка вместо
 * `onTimerFired`) НЕ КОМПИЛИРУЕТСЯ, той же механикой, что уже названа у `ActorBudgets` выше. Это не
 * тотальная гарантия: тот же обход применим и здесь — значение, собранное не литералом
 * (`{...a, onTimer: f}`, `as ActorHandlers`, объект, пришедший из JS-авторства без проверки типов),
 * excess-property-check не видит, и тогда не ловит ничем НИ валидатор, ни схема — хендлер просто не
 * позовут, молча. Стратегии здесь пишет LLM, для которой «помнить, что вот этот один вид называется
 * иначе» — ровно тот вид требования, который она нарушит молча на любом из этих обходных путей.
 *
...
export type ActorHandlers<S extends ActorStateValue = ActorStateValue> = {
   ...
   * с тем же классом отказа (LLM-автор держит исключение в памяти; промах ловит excess-property-check
   * ТОЛЬКО на свежем литерале, см. точность формулировки в doc правила выше — на любом обходном пути
   * не ловится ничем).
   ...
   * стратегии здесь пишет LLM: исключение придётся держать в памяти, и промах по нему на свежем
   * литерале ловит excess-property-check (см. точность формулировки в doc правила выше), а на любом
   * обходном пути (не литерал, `as ActorHandlers`, JS-авторство) — не ловится ничем; хендлер просто
   * не позовут, молча. Ветка ломающая по построению, одним переименованием её характер не меняется.
   */
