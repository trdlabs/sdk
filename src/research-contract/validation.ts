...
export type ValidationCode =
  ...
  // --- error (083 S3, аддитивно; ХОСТ не может исполнить объявленную форму стратегии) ---
  // Отличается от `lifecycle_form_invalid` тем, ЧЬЯ это проблема. `lifecycle_form_invalid` —
  // манифест сам себе противоречит (объявленная форма не совпадает с набором хуков), и виноват
  // автор. `unsupported_lifecycle` — манифест безупречен, а исполнить его здесь и сейчас нельзя:
  // раскатка не разрешена, режим исполнения хоста несовместим с объявленной формой, либо у
  // исполнителя нет нужной способности.
  //
  // ПОЧЕМУ ОТДЕЛЬНЫЙ КОД, А НЕ `invalid_module_ref`. Ref валиден и резолвится — подменять причину
  // значит отправлять автора чинить то, что не сломано. Потребитель, различающий по коду, обязан
  // отличить «манифест неверен» от «здесь его исполнить нельзя»: первое чинится правкой стратегии,
  // второе — конфигурацией хоста, и это разные люди.
  //
  // PATH У ЭТОГО КОДА — `''`. Нарушающего узла в запросе НЕТ: запрос корректен целиком, не
  // совпадает окружение. Указать `/moduleRef` значило бы обвинить валидный узел.
...
export interface ValidationIssue {
  ...
  /**
   * JSON Pointer (RFC 6901) к нарушающему узлу.
   *
   * **`''` — НОРМАТИВНАЯ ссылка на документ целиком**, а не «путь неизвестен» и не «поле забыли».
   * Так его определяет сам RFC 6901 (§5: пустая строка ссылается на весь документ), и так его
   * следует читать здесь: причина относится к запросу как таковому, нарушающего УЗЛА в нём нет.
   *
   * Это не украшение. Указать конкретный узел там, где узел валиден, значит отправить читателя
   * чинить то, что не сломано: `unsupported_lifecycle` возникает при безупречном манифесте и
   * валидном `moduleRef`, а не совпадает окружение хоста. `''` — единственный честный указатель
   * для такой причины.
   *
   * Поле остаётся ОБЯЗАТЕЛЬНЫМ. Опустить его значило бы завести второй способ сказать «узла нет»,
   * и потребителю пришлось бы различать `undefined` и `''`, которые означают одно и то же.
   */
