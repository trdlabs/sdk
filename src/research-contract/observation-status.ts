...
//    по ходу прогона, статическим каналом на старте актора. С задачи 5 канал НАСТОЯЩИЙ (не
//    «будущий», как утверждала эта строка после I-5 ревью): `ActorInit.subscriptions` —
//    `readonly ActorSubscriptionDescriptor[]` (`event-driven.ts`) — закрытый список разрешённых
//    подписок, известный актору до первого события. Вида нет в прогоне ⇔ его подписки нет в этом
//    списке — статический факт, читаемый ОДИН РАЗ на старте, а не по ходу прогона.
...
//    Это `ObservationStatus.gap` ниже: статус читается ИЗ ПРОЕКЦИИ (pull-API `ctx`, задача 5), а
