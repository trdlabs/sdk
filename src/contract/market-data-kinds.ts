 ...
 * 'funding', 'taker']`. **Поправка (фактическая точность диапазона):** `^0.13.0` НЕ подтянет
 * `0.14.0` автоматически — caret на нулевом мажоре пинит минор (`^0.13.0` = `>=0.13.0 <0.14.0`),
 * и `0.14.0` в этот диапазон не входит (semver). Значит потребитель НЕ ломается по факту публикации
 * этой версии — `npm install`/lockfile-резолюция на `^0.13.0` эту версию не увидят вовсе. Он
 * сломается только тогда, когда зависимость на `@trdlabs/sdk` осознанно поднимут (ручной бамп
 * диапазона в `package.json`, например `^0.13.0` → `^0.14.0`, либо на конкретную `0.14.0`+). Когда
 * это случится: `MarketDataKind` станет пятёркой snake_case, и `'openInterest'`/`'taker'` перестанут
 * быть её членами — ошибка СБОРКИ у потребителя в момент этого осознанного бампа, а не молчаливая
 * подмена (`'liquidations'`/`'funding'` совпадают по написанию, но это совпадение написания, не
 * совместимость смысла). Правка — на стороне `backtester` (перейти на `LegacyMarketDataKind` либо
 * на собственную локальную копию); в этой ветке она вне скоупа, правило ветки запрещает файлы вне
 * `sdk`.
 *
