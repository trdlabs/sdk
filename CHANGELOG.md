...
### 083 S3 ступень 1 — привязка требования к инструменту стала размеченным объединением

**`CONTRACT_VERSION` 017.5 → 017.6**: меняется ПОВЕРХНОСТЬ манифеста, а не форма ответа.

Требование к рыночным данным получает вторую ветвь привязки:

```ts
// фиксированная — сегодняшняя форма, не изменилась ни на бит
{ instrument: { venue, symbol } }
// связанная — венью названо, символ приносит прогон
{ instrument: { venue }, symbolFrom: 'actor' }
```

До этого `instrument` нёс два смысла сразу — идентичность рынка и привязку требования к прогону.
При одном акторе они совпадают и потому неразличимы; при N расходятся, и любое одно чтение меняет
смысл публичного поля.

Ключевое свойство: **ни одно поле не игнорируется и ни одно не подразумевается**. У связанной ветви
символа нет вовсе — значит нечего не соблюсти. Символ, названный рядом с `symbolFrom: 'actor'`,
ОТВЕРГАЕТСЯ, а не игнорируется молча.

Различитель — отдельное поле `symbolFrom`, а НЕ отсутствие символа: отсутствие ключа как
разрешающее значение неотличимо от потери поля. Потеря `symbolFrom` переводит требование в
фиксированную ветвь, где символ обязателен, и отказ наступает сразу.

**Совместимость.** Прежние манифесты не трогаются: фиксированная ветвь под `017.5` валидна ровно
как была. Порог `SYMBOL_FROM_MIN_CONTRACT_VERSION` (`017.6`) — ТРЕТИЙ порог рядом с двумя
существующими, а не переиспользованный: манифест, объявивший `017.5`, отвергается при попытке
воспользоваться формой, которой в `017.5` не было.

## [0.21.0] - 2026-08-17

### cc#365 — происхождение свечей в покрытии

Аддитивно: прежние формы не менялись, `CONTRACT_VERSION` остаётся `017.5`. Новое необязательное
поле `HistoricalCoverageEntry.candleOrigin` и новый разбор в `@trdlabs/sdk/historical`.

У открытого интереса и ликвидаций атрибуция источников в канонической строке есть
(`oiSourceMask`, `liqSourceMask`), а у СВЕЧИ не было ни одного поля: потребитель, получивший
сутки, не мог узнать, чья это цена, и не мог сверить её с требованием стратегии. Платформа
пишет происхождение в KV партиции в момент записи (pl#230); здесь едет его сторона на проводе.

**Сумма, а не пара полей.** Имя венью выразимо только внутри `kind: 'venue'`, а `venue` и
означает однородность — состояние «имя есть, про однородность неизвестно» не запрещено
проверкой, оно НЕПРЕДСТАВИМО. Пара независимых полей (`candleVenue?` +
`candleVenueHomogeneous?`) его допускала бы, и ловить пришлось бы на границе: при рассинхроне
версий, при частичной сериализации, у продюсера, который про правило не знал. Пока форма
допускает дурное состояние, оно рано или поздно возникает.

**`mixed` не сворачивается в `unknown`.** Перечень венью — это ЗНАНИЕ («источников было
несколько, вот они»), а не его отсутствие. Он уже вычислен тем, кто отказ породил; заставлять
владельца датасета выяснять состав заново значит терять сделанную работу.

**Пять причин отсутствия доказательства, и они двух родов.** Про архив — `no_files`,
`not_declared`, `invalid_value`: сервер посмотрел и говорит, что нашёл. Про канал —
`not_reported`, `malformed`: наблюдение самого клиента. Схлопнуть `not_reported` в
`not_declared` — самая дорогая здесь ошибка: она превращает «мы разговариваем с платформой
старше cc#365» в «у ваших данных нет происхождения», и владелец пойдёт чинить архив вместо
обновления. Канальные причины от сервера НЕ принимаются: сервер, произносящий `not_reported`,
себе противоречит — он не молчит.

Поле на проводе типизировано как `unknown` намеренно: клиент отдаёт тело ответа как есть, и
типизированное поле было бы утверждением, которого никто не проверял. Разбор —
`coverageEntryCandleOrigin` / `parseCandleOrigin`, fail-closed на каждом шаге.

Совместимость: платформа старше cc#365 поле не эмитит, и это законное состояние —
`unknown:not_reported`.

## [0.20.0] - 2026-08-17

### Д5 — общий оракул result digest переезжает в SDK

Аддитивно: прежние формы не менялись, `CONTRACT_VERSION` остаётся `017.5`. Новый экспорт из
`@trdlabs/sdk/historical`.

«Один оракул на все три пути» — условие осмысленности замера Д5, а не пожелание, и держится оно
только тем, что стороны берут **один и тот же код**. Стороны лежат в разных репозиториях:
platform отдаёт данные, а бэктестер при форме «файлы + Range» декодирует parquet сам и считает
digest по своим материализованным строкам. Platform не публикуемый пакет — из бэктестера до него
не дотянуться, — поэтому оракул, оставленный там, неизбежно породил бы вторую копию. Два
канонизатора расходятся на первом же краевом случае (`undefined` против `null`, `bigint` против
`number`, `-0`), и расхождение выглядело бы как дефект транспорта, то есть ровно как то, что
замер должен обнаруживать.

Добавлено:

- `computeResultDigest` — digest по **каноническому набору строк после декодирования**, а не по
  байтам ответа: wire-форма JSON, Arrow IPC и parquet различается по построению, и байтовое
  равенство между путями недостижимо в принципе.
- `checkParity` — **fail-closed** вердикт. Равенство путей необходимо, но недостаточно: три
  одинаково пустых или одинаково усечённых ответа согласуются идеально. Поэтому требуется
  независимое ожидание (точный `rowCount` и границы окна), а неполное ожидание — само по себе
  нарушение. `minRowCount` точный `rowCount` не заменяет.
- `checkAgainstExpectation`, `digestsAgree`, `canonicalRowLine`.
- `ResultDigestError` с именованным типом кода `ResultDigestFailure`: `DUPLICATE_ROW_KEY`,
  `NON_FINITE`, `PRECISION_LOST`, `TYPE_MISMATCH`. Внутри platform это было встроенное
  объединение — там его никто не импортировал; в публикуемом пакете потребитель, ветвящийся по
  `e.code`, обязан иметь возможность назвать тип в своей сигнатуре.

Оракул не чинит и не сглаживает: всякий вход, на котором паритет **неопределён**, — отказ, а не
значение по умолчанию.

Равенство с реализацией platform доказано **на значениях**, а не сравнением исходников: 15 из 15
случаев дают тот же digest (включая пустое множество, `undefined` как отсутствие поля и
bigint-штамп) и те же четыре кода отказа.

## [0.19.0] - 2026-08-13

### 083 S3 — канонический идентификатор хостового источника (ADR-0012, добор)

Аддитивно: формы не менялись, `CONTRACT_VERSION` остаётся `017.5`.

`0.18.0` завёл хостовый вариант дескриптора, но **значение** идентификатора оставил на усмотрение
хоста — а этого мало. Идентификатор наблюдаем автором: он приезжает в каждом конверте хостового
события, автор кладёт его в свой FSM и по нему ветвится. Выбери значение хост — бэктестер,
платформа и живая торговля выберут разные, и один и тот же код стратегии поведёт себя в трёх средах
по-разному, причём молча: форма у всех трёх одинаковая, и ни одна проверка формы этого не заметит.
Ровно это уже случилось: бэктестер придумал себе `'sub-internal'`, потому что взять значение было
неоткуда.

Добавлены:

- `HOST_SUBSCRIPTION_ID` — каноническое значение (`'host'`), выбирает его контракт, а не хост;
- `HOST_SOURCE_DESCRIPTOR` — замороженный готовый дескриптор, тот самый, что хост обязан положить
  в `ActorInit.subscriptions`;
- `isHostSubscriptionId(id)` — предикат для мест, где под рукой только конверт, а дескриптора нет.

Дескриптор заморожен и единственен: два экземпляра с одинаковым содержимым — это два места, где
значение может разойтись.

## [0.18.0] - 2026-08-13

### 083 S3 — два решения по контракту актора (`CONTRACT_VERSION` → `017.5`)

**Ломающее для формы `event_driven`.** Манифест, объявляющий actor-surface под `017.4`, теперь
отклоняется ровно так же, как раньше отклонялся под `017.3`: версия описывает форму, которой в
пакете больше нет. Форма `single_position` не затронута ни одним битом.

#### ADR-0012 — источники событий актора: закрытый список, хостовый источник объявлен явно

`ActorSubscriptionDescriptor` стал замкнутым union'ом: `ActorMarketSourceDescriptor` (несёт
`requirementId`) и `ActorHostSourceDescriptor` (`kind: 'host'`, требования манифеста нет). Добавлено
сужение `isHostSourceDescriptor`.

Причина: `ActorEnvelope` требует `subscriptionId` у КАЖДОГО события, а половина каталога §3.5
(`fill`, четыре ордерных события, `timer.fired`) рождается внутри хоста и подписки не имеет.
Значение взять было неоткуда — хост-реализация подставляла собственную строку. Автор, проверяющий
`subscriptionId` по списку источников (контракт разрешает ему именно это), увидел бы неизвестный
идентификатор и обязан был бы счесть событие подложным; второй хост выбрал бы другую строку, и один
и тот же код стратегии повёл бы себя на бэктесте и в проде по-разному, молча.

Разложение на два варианта, а не опциональное поле: опциональность позволила бы рыночному источнику
приехать без требования и не быть замеченным.

#### ADR-0013 — единица заявки: первичен нотионал, базовый размер — оценка

`OpenOrderViewBase`: `qtyUsd` и `filledQtyUsd` — обязательные факты в одной единице (остаток заявки
считается без всякой цены, предикат частичного исполнения — `0 < filledQtyUsd < qtyUsd`);
`estimatedQty?` — необязательная ОЦЕНКА базового размера; `filledQty?` — ФАКТ исполнения. Поле
`qty` упразднено.

Причина: `qty` объявлялось «размером, ПРИНЯТЫМ риском» — утверждение, за которым не стояло ни
риска, ни размера. У actor-пути риск-контура нет вовсе, а хост, у которого нет цены пересчёта,
подставлял ноль — и по прежней формуле остатка (`qty − filledQty`) это читалось как «исполнена
целиком», то есть автор видел заявку, которой уже нет.

#### Миграция

- манифесты `event_driven`: `contractVersion` → `017.5`;
- читатели `ActorInit.subscriptions`: разложить по виду перед чтением `requirementId`;
- читатели `ctx.orders.open()`: остаток считать в quote-единице; `qty` → `estimatedQty`/`filledQty`
  там, где базовая величина действительно нужна.

## [0.17.1] - 2026-08-13

### Исправлена ошибочная семантика тестового контракта: область отказа — ДЕНЬ

DTO и runtime API не менялись. Исправлен conformance-харнесс.

Первая редакция `runDayIntegrityConformance` требовала, чтобы окно того же дня без
стыка отвечало `200` (`cleanProbe`). Требование было **ошибочным** и развело две честные
реализации: платформа грузит закрытый день целиком и отказывала, мок фильтровал по окну и
отвечал. Неправа была не реализация, а контракт.

Недостоверен **день**. Знание об этом не должно зависеть от того, какое окно и какие
символы спросили: иначе потребитель получает строки дня, о ложности соседних строк в
котором уже известно, и не узнаёт об этом.

`seam` теперь принимает три необязательных поля вместо `cleanProbe`:

- `sameDayCleanWindow` — окно того же дня без стыка; обязано дать **тот же** отказ и **то же
  тело**;
- `otherSymbolSameDay` — символ того же дня без собственного дубля; обязан отказать, потому
  что недостоверен день, а не символ;
- `healthyOtherDay` — окно другого, заведомо исправного дня; обязано ответить `200`. Без
  него нельзя отличить работающий инвариант от сервиса, отказывающего на всё подряд.

## [0.17.0] - 2026-08-13

### Д3 — целостность ключа дня: типизированный отказ вместо голого `HTTP 409`

Аддитивно: существующие вызовы не меняются, поверхности добавляются рядом.

Контракт чтения объявляет `(minute_ts, symbol)` **тотальным порядком** — на нём построен
keyset-курсор `/historical/rows`. Платформа теперь проверяет это над всем загруженным днём и
отдаёт `409` с `code: DUPLICATE_ROW_KEY`, когда в дне две строки с одним ключом.

**Под `409` у этого эндпоинта живут два разных факта, и действия по ним противоположны.**
`generation changed` разрешается перечитыванием с начала; `DUPLICATE_ROW_KEY` перечитыванием не
разрешается вовсе, пока данные не починены. Свести их к одному статусу значит заставить
потребителя разбирать текст, а чаще — ретраить вечно то, что само не пройдёт.

**`HistoricalDayIntegrityError`** несёт адрес нарушения целиком: `date`, `symbol`, `minuteTs`,
`generation`. Без него «день недостоверен» нельзя ни проверить, ни передать в repair.

**`classifyDayIntegrityResponse(status, body)`** классифицирует по **точной тройке** — статус, код
и полный набор полей тела, — и дополнительно требует `permanent: true` с `retryFromStart: false`.
Постоянство проверяется, а не выводится из кода: ответ, называющий себя повторяемым, отказом
целостности не является, что бы ни стояло в `code`. Знакомый код с чужим статусом или в
повреждённом теле означает, что отвечал не наш сервис, — приняв такое за отказ, клиент перестал бы
повторять ровно там, где повтор и нужен.

`non-retryable` здесь — **локальное свойство одного вызова**, а не приговор операции: повтор этого
запроса даст тот же ответ, но после repair с evidence тот же период станет читаемым.

Сообщение ошибки намеренно сохраняет форму `<label>: HTTP <status>` — классификаторы, разбирающие
текст, продолжают работать, а типы приезжают дополнительно, а не вместо.

Тело ошибки читается только для `409`: для остальных статусов поведение прежнее, а непарсящееся
тело не роняет клиента — такой ответ просто не классифицируется.

**Conformance:** `runDayIntegrityConformance(target, seam)` сверяет обе стороны с выписанной руками
таблицей. Ядро проверки — четыре разных `limit` обязаны дать **один и тот же** ответ: зависимость
результата от размера страницы и была дефектом.
