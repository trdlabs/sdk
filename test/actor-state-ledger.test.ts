...
// I-3 (задача 6, ревью раунда 1, Important — обе таймингом-основанные проверки ниже переписаны).
// Было `elapsedMs < N`. Замер ревью: тёплая медиана 0.38мс, но холодный первый вызов на
// простаивающей машине — 11.6мс (запас всего 1.7x); под нагрузкой (4-8 busy-loop'ов) тест был
// красным в 3 из 6 / 12 из 12 прогонов. Миллисекунды мерили прогрев JIT и везение планировщика
// ОС, не заявленную сложность.
//
// Заменено на подсчёт вызовов `Set.prototype.add` за время СИНХРОННОГО вызова — детерминированная,
// не зависящая ни от машины, ни от нагрузки метрика, которая считает РОВНО ту операцию, чья
// стоимость и была источником обеих регрессий:
//   - O(глубина²) (тест ниже, линейная цепочка): старый баг копировал `new Set(ancestors)` НА
//     КАЖДОМ узле — копирование множества размера d вызывает `.add()` d раз (по разу на элемент),
//     то есть суммарно O(глубина²) вызовов; текущий backtracking зовёт `.add()` РОВНО один раз на
//     узел — O(глубина) вызовов;
//   - O(2^глубина) (DAG-тест дальше по файлу): `ancestors.add(obj)` достигается ТОЛЬКО когда memo
//     (`confirmed.get`, отдельная структура — Map, не Set) ещё НЕ разрешил узел, то есть ровно на
//     ПЕРВОМ посещении каждого РАЗЛИЧНОГО узла; без корректной мемоизации разделяемое поддерево
//     посещалось бы заново по каждому пути к нему, и счётчик `.add()`-вызовов рос бы
//     экспоненциально с уровнем ветвления вместо линейного роста с числом различных узлов.
// Эмпирически проверено (`node` против собранного `dist`, обе регрессии посчитаны вручную): текущая
// реализация — chain depth 100→400 даёт add-вызовов 101→401 (коэффициент ×3.97, линейно); DAG
// levels 23→30→50 даёт 24→31→51 (коэффициент ×2.13 на границах 23→50, линейно по числу различных
// узлов levels+1). Гипотетическая O(глубина²)-регрессия дала бы коэффициент ×~16 на той же паре
// глубин (копирование размера d стоит d вызовов на каждом из d узлов — сумма квадратична); порог
// ×8 ниже лежит ровно посередине между ×4 (линия) и ×16 (квадрат) и не зависит от точной константы
// накладных расходов на узел.
//
// Монки-патч `Set.prototype.add` восстанавливается в `finally` СРАЗУ после синхронного вызова —
// `isPlainActorState`/`isPlainDataValue` не делают ничего асинхронного, поэтому окно патча не
// пересекается ни с каким другим кодом (тестовым или продовым) в этом однопоточном процессе.
function countSetAdds<T>(fn: () => T): { readonly result: T; readonly setAdds: number } {
  let setAdds = 0;
  const original = Set.prototype.add;
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  (Set.prototype as any).add = function (this: Set<unknown>, value: unknown): Set<unknown> {
    setAdds += 1;
    return original.call(this, value);
  };
  try {
    const result = fn();
    return { result, setAdds };
  } finally {
    Set.prototype.add = original;
  }
}

test('isPlainActorState принимает вложенную структуру ДО потолка глубины — O(глубина), не O(глубина²)', () => {
  const shallow = countSetAdds(() => isPlainActorState(deepPlainObject(100)));
  const deep = countSetAdds(() => isPlainActorState(deepPlainObject(400)));
  assert.equal(shallow.result, true);
  assert.equal(deep.result, true);

  // Линейный рост: глубина ×4 (100→400) обязана дать add-вызовов ×~4 (эмпирически ×3.97), НЕ
  // ×~16 (квадрат, что дала бы регрессия к копированию Set). Порог ×8 — маркер регрессии, не
  // точная бюджетная цифра.
  const ratio = deep.setAdds / shallow.setAdds;
  assert.ok(
    ratio < 8,
    `O(глубина²) дал бы квадратичный рост Set.add-вызовов; глубина ×4 (100→400) дала ×${ratio.toFixed(2)} ` +
      `(100=${shallow.setAdds}, 400=${deep.setAdds})`,
  );
});
...
test('isPlainActorState: DAG с разделяемыми поддеревьями — O(узлы), не O(2^глубина)', () => {
  ...
  assert.ok(counts[23]! < counts[30]! && counts[30]! < counts[50]!, JSON.stringify(counts));
  // Линейный рост: levels 23→50 (×2.17, различных узлов levels+1 растёт так же) обязан дать
  // add-вызовов того же порядка (эмпирически ×2.13). Щедрый порог ×15 отделяет линейный рост от
  // экспоненциального (2^27 ≈ 134 миллиона раз при регрессии к отсутствию мемоизации) с огромным
  // запасом, оставаясь нечувствительным к точной константе накладных расходов на узел.
  const ratio = counts[50]! / counts[23]!;
  assert.ok(
    ratio < 15,
    `O(2^глубина) дал бы экспоненциальный рост add-вызовов; levels 23→50 дали ×${ratio.toFixed(2)} ` +
